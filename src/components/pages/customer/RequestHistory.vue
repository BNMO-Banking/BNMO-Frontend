...
const {
    requestHistories,
    pageMetadata,
    isLoadingRequestHistories,
    errorRequestHistories } = storeToRefs(requestStore)

requestStore.getRequestHistories(account.value.ID, page.value)
...
<template>
    <table class="mx-8 my-6">
        ...
        <tbody class="text-center">
            <RequestHistoryRow
                v-for="(data, index) in requestHistories"
                :key="data.ID"
                :data="data"
    ...
    <div class="flex w-full items-center justify-center">
        <svg v-if="isLoadingRequestHistories" aria-hidden="true" class="w-16 h-16 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-main-green" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <h3
        ...
        v-if="requestHistories.length === 0 && !isLoadingRequestHistories"
    >
        Request history empty. Start requesting
    </h3>
    <h3
        class="flex text-3xl text-main-red font-extrabold items-center justify-center w-full uppercase"
        v-if="errorRequestHistories"
    >
        {{ errorRequestHistories }}
    </h3>
    <div
        class="flex justify-center items-center gap-x-16 mb-4"
        v-if="requestHistories.length !== 0"
    >
        <button
